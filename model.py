...
class LidarCenterNet(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.longitudinal_controller = LongitudinalLinearRegressionController(self.config)
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command, target_point_next=None, timer=None):
    """
    timer: optional t_u.BlockTimer instance. When provided every sub-block is timed.
    """
    _t = timer  # shorthand; None → no timing overhead
    bs = rgb.shape[0]
    ...
    # ------------------------------------------------------------------ backbone
    if _t:
      with _t.measure('backbone/TOTAL'):
        if self.config.backbone in ('transFuser', 'bev_encoder'):
          bev_feature_grid, fused_features, image_feature_grid = self.backbone(rgb, lidar_bev, timer=_t)
        else:
          raise ValueError('The chosen vision backbone does not exist.')
    else:
      if self.config.backbone in ('transFuser', 'bev_encoder'):
    ...
    if self.config.use_wp_gru or self.config.use_controller_input_prediction:
      if self.config.transformer_decoder_join:
        # ------------------------------------------------ channel proj + pos enc
        if _t:
          with _t.measure('join/channel_proj_posenc'):
      ...
      # ------------------------------------------------ extra sensor encoding
      if self.extra_sensors:
        if _t:
          with _t.measure('join/extra_sensor_encoder'):
            extra_sensors = []
            if self.config.use_velocity:
      ...
      if self.config.transformer_decoder_join:
        ...
        # ------------------------------------------------ transformer decoder (WP)
        if self.config.use_wp_gru:
          if self.config.multi_wp_output:
            if _t:
              with _t.measure('join/transformer_decoder_wp'):
            ...
            if _t:
              with _t.measure('decoder/gru_wp'):
        ...
        if self.config.use_controller_input_prediction:
          if self.config.tp_attention:
            ...
            if _t:
              with _t.measure('join/transformer_decoder_checkpoint'):
                joined_checkpoint_features, attention = self.join(self.checkpoint_query.repeat(bs, 1, 1),
                                                                   fused_features)
            else:
          ...
          if _t:
            with _t.measure('decoder/gru_checkpoint'):
              pred_checkpoint = self.checkpoint_decoder(gru_features, target_point)
            with _t.measure('decoder/target_speed_network'):
              if self.config.input_path_to_target_speed_network:
                ts_input = torch.cat(
                    (target_speed_features, pred_checkpoint.reshape(bs, self.config.predict_checkpoint_len * 2)),
                    axis=1)
    ...
    if self.config.use_semantic:
      if _t:
        with _t.measure('head/semantic_decoder'):
    ...
    if self.config.use_depth:
      if _t:
        with _t.measure('head/depth_decoder'):
    ...
    if self.config.use_bev_semantic:
      if _t:
        with _t.measure('head/bev_semantic_decoder'):
    ...
    if self.config.detect_boxes:
      if _t:
        with _t.measure('head/centernet_detection'):
  ...
  def control_pid_direct(self,
    ...
    if self.make_histogram:
        # Ensure scalar for histogram
        if isinstance(pred_target_speed, torch.Tensor):
            pred_target_speed_scalar = pred_target_speed.item()
        else:
            pred_target_speed_scalar = float(pred_target_speed)

        self.speed_histogram.append(pred_target_speed_scalar * 3.6)

    # Convert speed tensor to scalar float
    if isinstance(speed, torch.Tensor):
        speed = speed[0].item()
    elif isinstance(speed, np.ndarray):
        speed = float(speed[0])

    # Convert target speed to scalar float
    if isinstance(pred_target_speed, torch.Tensor):
