...
def get_throttle(config, brake, target_speed, speed):
    target_speed = _to_scalar(target_speed)
    speed = _to_scalar(speed)
    brake = bool(_to_scalar(brake))
    ...
    # -------------------------------------------------
    # 🔑 FALLBACK FOR INFERENCE / CPU MODELS
    # -------------------------------------------------
    if params.size < 7:
        throttle = np.clip(speed_error / max(target_speed, 1e-3), 0.0, 1.0)
        return float(throttle), False

    threshold = float(params[-1])

    if (speed / max(target_speed, 1e-5)) > threshold or brake:
        return 0.0, True
    ...
    throttle = float(np.clip(features @ params[:-1], 0.0, 1.0))
