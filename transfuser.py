...
class TransfuserBackbone(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.img_pools = nn.ModuleList([
        nn.AvgPool2d(kernel_size=(8, 8), stride=(8, 8)),
    ...
    self.lidar_encoder = timm.create_model(config.lidar_architecture,
                                             ...
                                             features_only=True)
    self.lidar_pools = nn.ModuleList([
        nn.AvgPool2d(kernel_size=(8, 8), stride=(8, 8)),
        nn.AvgPool2d(kernel_size=(4, 4), stride=(4, 4)),
        nn.AvgPool2d(kernel_size=(2, 2), stride=(2, 2)),
        nn.Identity(),
  ...
  def forward(self, image, lidar, timer=None):
    '''
        Image + LiDAR feature fusion using transformers
        Args:
            image_list (list): list of input images
            lidar_list (list): list of input LiDAR BEV
            timer (BlockTimer): optional timing accumulator
        '''
    _t = timer  # shorthand; None means no timing
    ...
    for i in range(4):
      if _t:
        with _t.measure(f'backbone/img_stage{i}'):
          image_features = self.forward_layer_block(image_layers, self.image_encoder.return_layers, image_features)
        with _t.measure(f'backbone/lidar_stage{i}'):
          lidar_features = self.forward_layer_block(lidar_layers, self.lidar_encoder.return_layers, lidar_features)
        with _t.measure(f'backbone/gpt_fusion_stage{i}'):
    ...
    if self.config.transformer_decoder_join:
      ...
      # Global average without AdaptiveAvgPool (ONNX-safe)
      image_features = image_features.mean(dim=(-2, -1))   # [B, C]
      lidar_features = lidar_features.mean(dim=(-2, -1))   # [B, C]
    ...
    if self.config.detect_boxes or self.config.use_bev_semantic:
      if _t:
        with _t.measure('backbone/fpn_top_down'):
  ...
  def fuse_features(self, image_features, lidar_features, layer_idx):
    ...
    image_embd_layer = self.img_pools[layer_idx](image_features)
    lidar_embd_layer = self.lidar_pools[layer_idx](lidar_features)
